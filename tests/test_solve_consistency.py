"""
Tests for consistency between solve() and solve_direct() methods.

This module contains regression tests for the issue where solve_direct()
incorrectly reported problems as "unbounded" while solve() correctly found
optimal solutions.
"""
...
class TestSolveConsistency:
    """Test cases for solve() and solve_direct() consistency."""
    
    def test_example01_consistency(self):
        """
        Test that solve() and solve_direct() return consistent results for example01.
        
        This is a regression test for the issue where solve_direct() incorrectly
        reported the problem as "unbounded" while solve() correctly found an
        optimal solution with eta=[0.25, 0.75].
        """
        ...
        assert sol_direct.status == "optimal", \
            f"solve_direct() returned status '{sol_direct.status}', expected 'optimal'"
        assert sol_file.status == "optimal", \
            f"solve() returned status '{sol_file.status}', expected 'optimal'"
        
        # Both should have the same status
        assert sol_direct.status == sol_file.status, \
            f"Status mismatch: solve_direct()='{sol_direct.status}', solve()='{sol_file.status}'"
        
        # Both should have the same eta parameter
        np.testing.assert_allclose(sol_direct.eta, sol_file.eta,
                                   err_msg="eta mismatch between solve_direct() and solve()")
        ...
        np.testing.assert_allclose(sol_direct.eta, expected_eta,
                                   err_msg="solve_direct() returned incorrect eta")
        np.testing.assert_allclose(sol_file.eta, expected_eta,
                                   err_msg="solve() returned incorrect eta")
    ...
    def test_consistency_with_bounds(self):
        ...
        sol_direct = benpy.solve_direct(B, P, a=a, b=b, l=l, s=s, opt_dir=opt_dir)
        ...
        assert sol_direct.status == sol_file.status, \
            f"Status mismatch with bounds: solve_direct()='{sol_direct.status}', solve()='{sol_file.status}'"
    ...
    def test_consistency_maximization(self):
        ...
        # Solve with both methods
        sol_direct = benpy.solve_direct(B, P, b=b, l=l, opt_dir=opt_dir)
        ...
        # Should have consistent status
        assert sol_direct.status == sol_file.status, \
            f"Status mismatch for maximization: solve_direct()='{sol_direct.status}', solve()='{sol_file.status}'"
    ...
    def test_vertex_counts_match(self):
        ...
        # Solve with both methods
        sol_direct = benpy.solve_direct(B, P, a=a, l=l, opt_dir=1)
        ...
        assert sol_direct.num_vertices_upper == sol_file.num_vertices_upper, \
            f"num_vertices_upper mismatch: solve_direct()={sol_direct.num_vertices_upper}, solve()={sol_file.num_vertices_upper}"
        assert sol_direct.num_vertices_lower == sol_file.num_vertices_lower, \
            f"num_vertices_lower mismatch: solve_direct()={sol_direct.num_vertices_lower}, solve()={sol_file.num_vertices_lower}"
