...
int set_opt(opttype* opt, const int argc, char **argv)
{
	...
	for(;;)
	{
		...
		int option_index = 0;
		c = getopt_long (argc-1, argv+1, "hbpsf:o:k:L:l:m:M:A:a:E:e:", long_options, &option_index);
...
// number of objectives
	...
	if (opt->plot && vlp->q!=3)
		error(csa,"OFF file generation for problem with 3 objectives only - try again without option -p");
